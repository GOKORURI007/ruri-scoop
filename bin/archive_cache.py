...
def is_manifest_blacklisted(manifest_name):
    ...
    blacklist = [
        item.strip()
        for item in blacklist_env.replace("\n", ",").split(",")
        if item.strip()
    ]
...
def process_cache_files():
    ...
    for file_path in cache_dir.iterdir():
        ...
        if max_file_size:
            ...
            if file_size > max_file_size:
                print(
                    f"Skipping {filename} - size {file_size} exceeds limit {max_file_size}"
                )
                continue
    ...
    for manifest_name, files in files_to_process.items():
        identifier = f"ruri-scoop-{manifest_name}"
        ...
        metadata = {
            "mediatype": "software",
            "title": f"{manifest_name} - ruri-scoop",
            "description": f"",
            "collection": "open_source_software",
            "subject": ["scoop", "ruri-scoop"],
            "bucket": "ruri-scoop",
            "repo": "https://github.com/gokoruri007/ruri-scoop",
        ...
        try:
            ...
            print(
                f"Successfully uploaded {len(files_to_upload)} file(s) to {identifier}"
            )
